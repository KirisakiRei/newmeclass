...
export default function YayasanForgotPassword() {
  ...
  const handleSubmit = async (event) => {
    ...
    setLoading(true);
    try {
      await yayasanAPI.forgotPassword(email);
      setSent(true);
      toast({
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#1a1a1a] to-[#2a2a2a] flex items-center justify-center py-12 px-4">
      <Card className="w-full max-w-md bg-[#2a2a2a] border-yellow-400/20">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-white text-2xl">Reset Password Yayasan</CardTitle>
          <CardDescription className="text-gray-400">
            Masukkan email yayasan Anda. Kami akan mengirim link untuk membuat password baru.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="text-gray-400 text-sm" htmlFor="yayasan-forgot-email">Email</label>
              <div className="relative">
                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 w-4 h-4" />
                <Input
                  id="yayasan-forgot-email"
                  type="email"
                  required
                  value={email}
