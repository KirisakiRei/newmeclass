...
export default function MitraResetPassword() {
  ...
  const handleSubmit = async (event) => {
    ...
    setLoading(true);
    try {
      await mitraAPI.resetPassword(token, formData.password);
      toast({
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#1a1a1a] to-[#2a2a2a] flex items-center justify-center py-12 px-4">
      <Card className="w-full max-w-md bg-[#2a2a2a] border-yellow-400/20">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-white text-2xl">Password Baru Mitra</CardTitle>
          <CardDescription className="text-gray-400">
            Buat password baru untuk akun mitra Anda.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="text-gray-400 text-sm" htmlFor="mitra-reset-password">Password Baru</label>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 w-4 h-4" />
                <Input
                  id="mitra-reset-password"
            ...
            <div>
              <label className="text-gray-400 text-sm" htmlFor="mitra-reset-confirm-password">Konfirmasi Password</label>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 w-4 h-4" />
                <Input
                  id="mitra-reset-confirm-password"
