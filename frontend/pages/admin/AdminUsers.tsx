      ...
      <Dialog open={roleDialogOpen} onOpenChange={(open) => (!open ? closeRoleDialog() : setRoleDialogOpen(true))}>
        <DialogContent className="max-h-[90vh] max-w-5xl overflow-y-auto border-yellow-400/20 bg-[#2a2a2a]">
          ...
          <form onSubmit={saveRole} className="space-y-6">
            ...
            <div className="space-y-4">{catalog.map((group) => <Card key={group.groupKey} className="border-yellow-400/10 bg-[#1a1a1a]"><CardHeader><CardTitle className="text-base text-white">{group.groupLabel}</CardTitle></CardHeader><CardContent className="space-y-4">{group.pages.map((page) => { const requiresBaseAccess = page.actions.some((action) => action.actionKey === 'view'); return <div key={page.pageKey} className="rounded-xl border border-yellow-400/10 bg-[#121212] p-4"><div className="mb-3 flex items-start justify-between gap-3"><div><p className="font-medium text-white">{page.pageLabel}</p></div>{requiresBaseAccess ? <span className="inline-flex items-center gap-1 rounded-full bg-yellow-400/10 px-2.5 py-1 text-[11px] text-yellow-200"><Wrench className="h-3.5 w-3.5" />Aktifkan akses dasar terlebih dahulu</span> : null}</div><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">{page.actions.map((action) => { const checked = roleForm.permissionKeys.includes(action.key); const disabled = roleLocked || (action.actionKey !== 'view' && isPermissionActionLocked(roleForm.permissionKeys, action.key)); return <label key={action.key} className={`flex items-center gap-3 rounded-lg border px-3 py-2 text-sm ${checked ? 'border-yellow-400/40 bg-yellow-400/10 text-yellow-100' : 'border-yellow-400/10 bg-[#1a1a1a] text-gray-300'} ${disabled ? 'opacity-50' : 'cursor-pointer hover:border-yellow-400/30'}`}><input type="checkbox" checked={checked} disabled={disabled} onChange={(event) => updatePermission(action.key, event.target.checked)} className="h-4 w-4 accent-yellow-400" /><div><p className="font-medium">{action.actionLabel}</p></div></label>; })}</div></div>; })}</CardContent></Card>)}</div>
